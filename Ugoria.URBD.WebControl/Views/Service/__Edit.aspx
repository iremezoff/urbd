...
<asp:Content ID="Content2" ContentPlaceHolderID="MainContent" runat="server">
    <script language="javascript" type="text/javascript">
        $(document).ready(function () {
            ...
            $(".servicename, .path1c").editable(function(value, settings) {
    ...
    <div class="content">
        ...
        <%Html.BeginForm(); %>
        <%=Html.Hidden("service.ServiceId", ViewData["service_id"])%>
        <table class="configuration">
            <tr>
                <td class="param-name">
                    <%=Html.Label("Name","Имя сервиса: ") %>
                </td>
                <%=Html.Hidden("service.Name", ViewData["service_path1c"])%>
                <td class="servicename"><%=ViewData["service_name"]%>
                </td>
            </tr>
            <tr>
                <td class="param-name">
                ...
                <%=Html.Hidden("service.Path1c", ViewData["service_path1c"])%>
                <td class="path1c"><%=ViewData["service_path1c"]%></td>
            </tr>
        </table>
        <%if(ViewBag.allow_change) {%>
