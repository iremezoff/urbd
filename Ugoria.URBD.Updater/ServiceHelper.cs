...
namespace Ugoria.URBD.Updater
{
    public static class ServiceHelper
    {
        ...
        private static extern int QueryServiceConfig(IntPtr hService, IntPtr lpQueryConfig, int cbBufSize, ref int pcbBytesNeeded);
        ...
        public static string GetServicePath(string ServiceName)
        {
            ...
            try
            {
                ...
                try
                {
                    return GetServicePath(hService);
                }
                finally
                {
                    CloseServiceHandle(scman);
        ...
        private static string GetServicePath(IntPtr hService)
        {
            QUERY_SERVICE_CONFIG qscs = new QUERY_SERVICE_CONFIG();
            IntPtr qscPtr = Marshal.AllocCoTaskMem(0);
            int bytesNeeded = 5;
            int retCode = QueryServiceConfig(hService, qscPtr, 0, ref bytesNeeded);
            if (retCode == 0 && bytesNeeded == 0)
            {
                throw new ApplicationException("Failed to query service path.");
            }

            qscPtr = Marshal.AllocCoTaskMem(bytesNeeded);
            retCode = QueryServiceConfig(hService, qscPtr, bytesNeeded, ref bytesNeeded);
            if (retCode == 0)
            {
                throw new ApplicationException("Failed to query service path.");
            }
            qscs = (QUERY_SERVICE_CONFIG)Marshal.PtrToStructure(qscPtr, typeof(QUERY_SERVICE_CONFIG));
            string ret = Marshal.PtrToStringAuto(qscs.lpBinaryPathName);
